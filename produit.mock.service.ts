...
export class ProduitMockService {
  ...
  private handleError(error: Response | any) {
   ...
   AjouterProduit (produit): Observable<any> {
    console.log(produit);
    return this.http.post<any>(this.urlProduits, JSON.stringify(produit), this.httpOptions).pipe(
      tap((p) => console.log(`added product w/ id=${p.id}`)));
  }

  majProduit (id, product): Observable<any> {
    return this.http.put(this.urlProduits + '/' + id, JSON.stringify(product), this.httpOptions).pipe(
  ...
  produitParId (id): Observable<any> {
    return this.http.get<any>(this.urlProduits + '/' + id, this.httpOptions).pipe(
      tap(_ => console.log(`get product id=${id}`))
    );
  }
